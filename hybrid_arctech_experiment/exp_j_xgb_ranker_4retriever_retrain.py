# hybrid_arctech_experiment/exp_j_xgb_ranker_4retriever_retrain.py
# ============================================================
# Final XGBoost Ranker Retraining - BATCH RESUME
#
# Candidate Retriever
# ------------------------------------------------------------
...
FEATURE_CACHE_PATH = (
    CACHE_DIR
    / "xgb_4retriever_full14_features.parquet"
)

CHECKPOINT_PATH = (
    CACHE_DIR
    / "xgb_4retriever_full14_features_checkpoint.parquet"
)
...
def build_or_load_feature_cache(
    *,
    ...
    if FEATURE_CACHE_PATH.exists():
        ...
        print(
            "\n===== 4-Retriever Feature Cache Load ====="
        )
    ...
    print(
        "\n===== 4-Retriever Feature Cache Build (BATCH) ====="
    )
...
def main():
    ...
    print(
        " Final XGB Retraining - 4 Retriever (BATCH RESUME)"
    )
    ...
    if overlap:
        ...
        raise ValueError(
            "LTR 1000과 Final 400 user가 겹칩니다: "
            f"{len(overlap)}"
        )
    ...
    target_user_ids = np.concatenate(
        [
            ltr_user_ids,
            final_user_ids,
        ]
    ...
    # 2. Runtime Engine
    #
    # XGB는 지금 재학습할 것이므로 main의 기존 XGB 모델을
    # load하지 않도록 temporary item mode로 engine만 생성한다.
    # ========================================================
    ...
    print(
        "기존 checkpoint는 유지하고 남은 사용자만 batch 방식으로 계산합니다."
    )


    # ========================================================
    # 3. New architecture feature cache
    # ========================================================

    feature_df = build_or_load_feature_cache(
    ...
    # ========================================================
    # 6. Final production train on all 1000
    # ========================================================
    ...
    # validation model no longer needed
    ...
    metadata = {
        ...
        "ranker": {
            ...
            "retriever_count_semantics":
                (
                    "Item + BPR + Content + User "
                    "candidate source count"
                ),

            "is_item_candidate_is_xgb_feature":
                False,
    ...
    print(
        "main.build_features()의 retriever_count도 "
        "Item+BPR+Content+User 4개를 세도록 바꿔야 합니다."
    )
