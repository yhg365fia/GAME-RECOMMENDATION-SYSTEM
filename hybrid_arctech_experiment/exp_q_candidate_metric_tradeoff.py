# hybrid_arctech_experiment/exp_q_candidate_metric_tradeoff.py
# ============================================================
...
# Optuna를 사용하지 않는다.
#
# O2 Joint Optuna의 최종 Winner에서
# XGBoost parameter / feature set을 완전히 고정하고,
#
# Candidate Size:
...
#   content20_user10
#
# 총 9 x 4 = 36 candidate config를 동일한 5-Fold User CV로 비교한다.
#
# 각 config에서 실제 validation Top-10 recommendation을 생성하고:
#
# - Macro P@10
# - Macro R@10
# - HR@10
...
# IMPORTANT
# ------------------------------------------------------------
# 이 실험은 "최고 모델 선택" 실험이 아니다.
#
# NDCG를 올리기 위해 Recall이 어느 지점부터 급격히 희생되는지
# trade-off / Pareto 구간을 확인하고,
# 다음 Constrained Optuna에서 사용할 Recall guardrail을
# 데이터로 정하기 위한 진단 실험이다.
#
# O2 Winner fixed:
# - Feature count = 20
...
TRADEOFF_N_ESTIMATORS = 94


# O2 final winner Trial 51
...
def tradeoff_feature_columns():
    ...
    # O2 Trial 51 fixed feature configuration
    features.append(
        "is_item_candidate"
    )
    ...
    # Retriever count remains in Full14.
...
def run_tradeoff_diagnostic(
    *,
    ...
    print(
        " Candidate Metric Trade-off Diagnostic"
    )
...
def tradeoff_main():
    ...
    folds_path = (
        TRADEOFF_RESULT_DIR
        /
        "candidate_metric_tradeoff_folds.csv"
    )
    ...
    summary_path = (
        TRADEOFF_RESULT_DIR
        /
        "candidate_metric_tradeoff_summary.csv"
    )
    ...
    size_path = (
        TRADEOFF_RESULT_DIR
        /
        "candidate_size_metric_aggregate.csv"
    )
    ...
    pareto_path = (
        TRADEOFF_RESULT_DIR
        /
        "ndcg_recall_pareto_frontier.csv"
    )
    ...
    config_path = (
        TRADEOFF_RESULT_DIR
        /
        "tradeoff_experiment_config.json"
    )
        ...
        json.dump(
            {
                ...
                "purpose":
                    (
                        "Measure end-to-end Top10 metric trade-offs "
                        "before defining Recall guardrails."
                    ),
    ...
    print(
        "2) O2 Winner의 Feature / XGB parameter / n_estimators=94를 고정했습니다."
    )
    ...
    print(
        "5) 이 결과에서 Recall/NDCG trade-off와 Pareto frontier를 보고 다음 Optuna guardrail을 정하세요."
    )
