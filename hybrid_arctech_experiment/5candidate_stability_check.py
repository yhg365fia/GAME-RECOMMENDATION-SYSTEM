# hybrid_arctech_experiment/five_candidate_stability_check.py
# ============================================================
# Experiment V - Final Five Candidate Stability Check
#
# 목적
# ------------------------------------------------------------
...
#     (PopAffinity 제거)
#
# 을 새로운 5-Fold user split에서 다시 검증한다.
#
# 핵심 원칙
# ------------------------------------------------------------
# - Candidate Size 고정
...
# - XGBoost random_state = 기존과 동일(42)
# - LTR users = 동일한 1000명
# - 바꾸는 것은 Fold split seed 하나뿐
#
# Original Fold Seed : 42
# New Fold Seed      : 20260926
#
# 따라서 이 실험은 새로운 튜닝이 아니라
# "현재 최종 후보들이 다른 사용자 fold 분할에서도
#  비슷한 성능/성격을 유지하는가?"를 확인하는 안정성 검사이다.
#
# IMPORTANT
# ------------------------------------------------------------
# - Optuna 사용 안 함
...
# - 이 실험이 끝난 뒤 Selection200으로 5개 후보를 비교한다.
# ============================================================
...
def main():
    ...
    print(
        (
            f" Original fold seed={ORIGINAL_FOLD_SEED} "
            f"-> New fold seed={STABILITY_FOLD_SEED}"
        )
    )
    ...
    # ========================================================
    # 6. New stability folds
    # ========================================================

    folds = make_user_folds(
        ltr_user_ids,
        STABILITY_CV_FOLDS,
        seed=STABILITY_FOLD_SEED,
    )
    ...
    # ========================================================
    # 7. Evaluate five final candidates
    # ========================================================
    ...
    for idx, (
        ...
        summary, candidate_fold_rows = (
            run_one_stability_candidate(
                model_key=
                    model_key,
    ...
    # ========================================================
    # 8. Compare with original fold seed
    # ========================================================
    ...
    compare_df = add_original_comparison(
        stability_df
    )


    compare_df = add_rank_comparison(
        compare_df
    )
    ...
    summary_path = (
        STABILITY_RESULT_DIR
        /
        "five_candidate_stability_summary.csv"
    )

    folds_path = (
        STABILITY_RESULT_DIR
        /
        "five_candidate_stability_folds.csv"
    )

    compare_path = (
        STABILITY_RESULT_DIR
        /
        "five_candidate_stability_compare_original.csv"
    )

    config_path = (
        STABILITY_RESULT_DIR
        /
        "five_candidate_stability_config.json"
    )
        ...
        json.dump(
            {
                ...
                "method":
                    (
                        "Only user fold split seed changed. "
                        "Candidate, features and XGBoost hyperparameters fixed."
                    ),
            },
            file,
            ensure_ascii=False,
    ...
    print(
        " STABILITY CHECK - NEW SEED RESULTS"
    )
    ...
    # 11. Two-seed average
    #
    # 탐색용 score가 아니라 안정성 해석용 참고값.
    # Selection200 선택 전에 전체적인 consistency를 보기 위한 값.
    # ========================================================
    ...
    two_seed_path = (
        STABILITY_RESULT_DIR
        /
        "five_candidate_two_seed_average.csv"
    )
    ...
    print(
        "\n===== TWO-SEED AVERAGE (REFERENCE ONLY) ====="
    )

    print(
        two_seed_df.to_string(
            index=False
        )
    )
    ...
    print(
        "1) Candidate Size / Ratio / Feature / XGB parameter는 모두 고정했습니다."
    )

    print(
        "2) E는 Ablation winner인 15-feature(-PopAffinity) 버전입니다."
    )

    print(
        f"3) Fold split seed만 {ORIGINAL_FOLD_SEED} -> {STABILITY_FOLD_SEED}로 바꿨습니다."
    )

    print(
        "4) XGBoost random_state는 기존과 동일한 42입니다."
    )

    print(
        "5) Final400 / Selection200 / Final Test200은 전혀 사용하지 않았습니다."
    )

    print(
        "6) 이 결과 확인 후 Final400을 Selection200 + Final Test200으로 분리하면 됩니다."
    )
