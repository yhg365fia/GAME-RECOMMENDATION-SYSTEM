# hybrid_arctech_experiment/exp_o2_joint_optuna_low_size_search.py
# ============================================================
# Experiment O2 - Joint Optuna Low-Size Search
#
# Candidate Retriever
# ------------------------------------------------------------
...
def main():
    ...
    print(
        " Candidate Size 40~70 + Ratio + Feature Groups + XGBoost"
    )
    ...
    print(
        f" Recheck: Top {TOP_TRIALS_TO_RECHECK} | "
    ...
    # ========================================================
    # 3. Load train/test once
    #    - train: popularity profile
    #    - test : candidate recall diagnostic
    # ========================================================
    ...
    # LTR 1000명의 positive test interactions
    ltr_user_set = set(
        np.asarray(
            ltr_user_ids
        ).tolist()
    ...
    # ========================================================
    # 4. Attach popularity profile ONCE
    # ========================================================
    ...
    # 첫 실행 때 이전 Joint Optuna의 최종 5-fold winner
    # (Trial 49)를 anchor로 넣는다.
    #
    # 기존 winner:
    # Size60 / content25
    # ItemFlag=True
    ...
    #
    # XGB:
    # depth=6, child=6, lr≈0.06228
    # subsample≈0.95377, colsample≈0.76399
    # lambda≈0.98366, alpha≈0.88992
        ...
        study.enqueue_trial(
            {
                ...
                "max_depth": 6,
                "min_child_weight": 6,
                "learning_rate": 0.06227680559851586,
                "subsample": 0.9537668792750537,
                "colsample_bytree": 0.7639900136538523,
                "reg_lambda": 0.9836610807868512,
                "reg_alpha": 0.8899227928266088,
            }
        )

        # 같은 winner 설정을 Size50에도 한 번 고정 투입해
        # "후보 수만 줄였을 때"의 효과를 빠르게 확인한다.
        study.enqueue_trial(
            {
                "candidate_size": 50,
                "ratio_option": "content25",
    ...
    # ========================================================
    # 8. Top trials -> Full 5-Fold recheck
    # ========================================================
    ...
    print(
        f" TOP {TOP_TRIALS_TO_RECHECK} FULL 5-FOLD RECHECK"
    )
    ...
    for rank_idx, frozen_trial in enumerate(
        ...
        print(
            f"[{rank_idx}/{min(TOP_TRIALS_TO_RECHECK, len(completed_trials))}] "
            f"Trial {frozen_trial.number} "
            f"| Search={float(frozen_trial.value):.6f}"
        )
        ...
        print(
            "5-Fold Mean:",
            f"{summary['recheck_5fold_mean_ndcg']:.6f}",
            "| Std:",
            f"{summary['recheck_5fold_std_ndcg']:.6f}",
            "| Candidate Recall:",
            f"{summary['candidate_pool_recall']:.4f}",
        )
    ...
    # ========================================================
    # 9. Best selection + final LTR1000 retrain
    # ========================================================
    ...
    print(
        "Best 5-Fold NDCG@10:",
        f"{best_summary['recheck_5fold_mean_ndcg']:.6f}",
    )
    ...
    print(
        "Candidate Recall:",
        f"{best_summary['candidate_pool_recall']:.4f}",
    )
    ...
    print(
        "1) Optuna objective = user-level CV NDCG@10."
    )

    print(
        "2) Candidate Recall은 diagnostic일 뿐 선택 objective가 아닙니다."
    )

    print(
        "3) Search는 3-fold + pruning, 상위 trial만 정식 5-fold로 재검증했습니다."
    )

    print(
        "4) Final 400은 전혀 사용하지 않았습니다."
    )

    print(
        "5) Final400 평가는 이 결과를 확정한 뒤 마지막에 1회만 수행하세요."
    )
