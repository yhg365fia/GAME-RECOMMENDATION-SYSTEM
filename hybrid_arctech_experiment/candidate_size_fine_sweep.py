# hybrid_arctech_experiment/exp_k3_xgb_low_candidate_size_fine_sweep.py
# ============================================================
# Experiment K3 - Low Candidate Size Fine Sweep
#
# Candidate Retriever
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# Low Candidate Size Fine Sweep
#
# K2 결과에서 candidate size가 작을수록 NDCG가 좋아졌고,
# 탐색 최저 경계였던 90이 최적이었다.
#
# 따라서:
#   70 / 75 / 80 / 85 / 90
#
# 을 5단위로 세밀하게 확인한다.
#
# Ratio는 K2 Size90 상위권 + aggregate 상위권 4개만 사용:
#   1) user20              = 45/20/15/20
#   2) content20_user10    = 50/20/20/10
#   3) baseline            = 50/20/15/15
#   4) content25           = 45/20/25/10
#
# K2에서 이미 만든 superset cache를 그대로 재사용한다.
...
def scale_ratio_to_size(
    ...
    largest remainder 방식으로 정수 quota로 변환.
    """
...
# K2 superset가 훨씬 크므로 실제로 이 값은 새 cache 생성에 사용하지 않는다.
ITEM_N = max(
    config[
        "item"
    ]
...
RESULT_DIR = (
    SAVED_MODEL_DIR
    / "results"
    / "xgb_4retriever_low_size_fine_sweep"
)
...
def run_ratio_size_sweep(
    *,
    ...
    print(
        " 4-Retriever Low Candidate Size Fine Sweep"
    )
    ...
    fold_path = (
        RESULT_DIR
        /
        "low_size_fine_sweep_fold_results.csv"
    )

    summary_path = (
        RESULT_DIR
        /
        "low_size_fine_sweep_summary.csv"
    )
    ...
    size_path = (
        RESULT_DIR
        /
        "low_size_candidate_size_summary.csv"
    )
    ...
    ratio_path = (
        RESULT_DIR
        /
        "low_size_base_ratio_summary.csv"
    )
...
def main():
    ...
    print(
        " Candidate sizes = 70 / 75 / 80 / 85 / 90"
    )

    print(
        " Ratios = Top 4 from K2"
    )
    ...
    if not k2_cache_path.exists():

        raise FileNotFoundError(
            "K2 superset cache가 없습니다.\n"
            "먼저 exp_k2_xgb_4retriever_ratio_size_sweep.py를 완료하세요.\n"
            f"Expected: {k2_cache_path}"
        )


    print(
        "\n===== K2 Superset Cache Load ====="
    )
    ...
    if missing:
        ...
        raise ValueError(
            "K2 superset cache에 retrieval rank가 없습니다: "
            f"{missing}"
        )
    ...
    (
        ...
        # candidate_pool_recall 계산용 test_df가 필요하므로
        # split만 로드한다.
        test_df=
            runtime_main.load_mf_split()[1],
    )
    ...
    print(
        "1) Final 400은 사용하지 않았습니다."
    )

    print(
        "2) K2 superset cache를 재사용했으므로 후보 생성은 다시 하지 않았습니다."
    )

    print(
        "3) Size 70/75/80/85/90을 5단위로 비교했습니다."
    )

    print(
        "4) 다음 Feature Ablation은 여기서 선택된 best_ratio_size.json을 사용하세요."
    )
