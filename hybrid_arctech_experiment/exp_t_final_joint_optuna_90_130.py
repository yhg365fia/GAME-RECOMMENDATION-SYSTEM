# hybrid_arctech_experiment/exp_t_final_joint_optuna_90_130.py
# ============================================================
# Experiment T - Final Joint Optuna 90-130
#
# 목적
# ------------------------------------------------------------
# Candidate Size / Retriever Ratio / Feature Groups / XGBoost
# hyperparameter를 Joint Optuna로 탐색한다.
#
# 단, NDCG만 올리면서 Precision / Recall을 희생하지 않도록
# actual validation Top-10 metric에 guardrail을 둔다.
#
# Search:
...
# - 80 Optuna trials
# - 3-fold user CV + pruning
#
# Search guardrail:
# - Macro P@10 >= 0.1065
# - Macro R@10 >= 0.1065
#
# 위 조건을 만족한 trial만 NDCG@10으로 경쟁한다.
#
# Recheck:
# - Search-feasible Top 10
# - Full 5-fold user CV
#
# Final guardrail:
# - Macro P@10 >= 0.1070
...
# - Final guardrail 만족
# - 그중 Macro NDCG@10 최대
# - tie-break: Recall -> Precision -> MAP -> lower NDCG std
#
# 함께 기록:
# - Macro P@10
# - Macro R@10
...
def main():
    ...
    print(
        " Candidate Size 90~130(step1) + Ratio + Feature Groups + XGBoost"
    )

    print(
        f" Search: {N_TRIALS} trials | "
        f"{SEARCH_CV_FOLDS}-fold + pruning"
    )

    print(
        (
            " Search Guardrail: "
            f"P@10 >= {SEARCH_PRECISION_GUARDRAIL:.4f} "
            f"| R@10 >= {SEARCH_RECALL_GUARDRAIL:.4f}"
        )
    )

    print(
        f" Recheck: Search-feasible Top {TOP_TRIALS_TO_RECHECK} "
        f"| {FINAL_RECHECK_CV_FOLDS}-fold"
    )

    print(
        (
            " Final Guardrail: "
            f"P@10 >= {FINAL_PRECISION_GUARDRAIL:.4f} "
            f"| R@10 >= {FINAL_RECALL_GUARDRAIL:.4f}"
        )
    )

    print(
        " Primary objective among feasible trials = Macro NDCG@10"
    )
    ...
    # Anchor trials
    #
    # Trade-off diagnostic에서 실제 Top10 기준으로 좋았던
    # candidate 구조 3개를 O2 Trial51 feature/XGB 설정과 함께
    # 첫 trials로 넣어 feasible baseline을 확보한다.
    # --------------------------------------------------------
    ...
    if len(
        ...
        # Anchor A: latest upper-bound sweep Ranking형
        # Size110 / user20
        study.enqueue_trial(
            {
                **anchor_common,
        ...
        # Anchor B: latest upper-bound sweep Hit/Recall형
        # Size120 / content20_user10
        study.enqueue_trial(
            {
                **anchor_common,
                "candidate_size": 120,
        ...
        # Anchor C: strong lower-side point
        # Size100 / user20
        study.enqueue_trial(
            {
                **anchor_common,
    ...
    if not feasible_completed_trials:
        ...
        print(
            "3-fold Search Guardrail을 만족한 trial이 없습니다."
        )

        print(
            "결과 CSV는 저장되었지만 Top8 재검증/최종 모델 학습은 수행하지 않습니다."
        )
    ...
    print(
        "P@10:",
        f"{search_best.user_attrs['search_precision_at_10']:.6f}",
    )

    print(
        "R@10:",
        f"{search_best.user_attrs['search_recall_at_10']:.6f}",
    )

    print(
        "NDCG@10:",
        f"{search_best.user_attrs['search_ndcg_at_10']:.6f}",
    )

    print(
        "MAP@10:",
        f"{search_best.user_attrs['search_map_at_10']:.6f}",
    )
    ...
    # ========================================================
    # 8. Search-feasible Top8 -> Full 5-Fold
    # ========================================================
    ...
    print(
        f" SEARCH-FEASIBLE TOP {len(trials_to_recheck)} "
        "FULL 5-FOLD RECHECK"
    )
    ...
    for rank_idx, frozen_trial in enumerate(
        trials_to_recheck,
        start=1,
    ):
        ...
        print(
            f"[{rank_idx}/{len(trials_to_recheck)}] "
            f"Trial {frozen_trial.number} "
            f"| Search NDCG="
            f"{frozen_trial.user_attrs['search_ndcg_at_10']:.6f}"
        )
        ...
        recheck_summaries.append(
            summary
        )

        recheck_fold_rows.extend(
            fold_rows
        )
        ...
        print(
            "5-Fold:"
            f" P={summary['recheck_5fold_precision_at_10']:.4f}"
            f" | R={summary['recheck_5fold_recall_at_10']:.4f}"
            f" | HR={summary['recheck_5fold_hit_rate_at_10']:.4f}"
            f" | NDCG={summary['recheck_5fold_ndcg_at_10']:.4f}"
            f" | MAP={summary['recheck_5fold_map_at_10']:.4f}"
            f" | CandRecall={summary['candidate_pool_recall']:.4f}"
            f" | Feasible={summary['final_feasible']}"
        )
    ...
    # Final guardrail 통과 모델을 먼저,
    # 그 안에서 NDCG -> Recall -> Precision -> MAP -> std 순.
    recheck_df = (
        recheck_df
        .sort_values(
            [
    ...
    recheck_folds_df = pd.DataFrame(
        recheck_fold_rows
    )


    recheck_summary_path = (
        OPTUNA_RESULT_DIR
        /
        "top_trials_5fold_recheck_summary.csv"
    )
    ...
    recheck_folds_path = (
        OPTUNA_RESULT_DIR
        /
        "top_trials_5fold_recheck_folds.csv"
    )
    ...
    if len(
        ...
        print(
            (
                "Top recheck trials 중 "
                f"P@10 >= {FINAL_PRECISION_GUARDRAIL:.4f} "
                f"및 R@10 >= {FINAL_RECALL_GUARDRAIL:.4f} "
                "를 동시에 만족한 모델이 없습니다."
            )
        )

        print(
            "5-fold 결과는 모두 저장했지만 최종 모델은 자동 확정하지 않습니다."
        )
    ...
    print(
        " CONSTRAINED JOINT OPTUNA COMPLETE"
    )
    ...
    print(
        "5-Fold P@10:",
        f"{best_summary['recheck_5fold_precision_at_10']:.6f}",
    )

    print(
        "5-Fold R@10:",
        f"{best_summary['recheck_5fold_recall_at_10']:.6f}",
    )

    print(
        "5-Fold NDCG@10:",
        f"{best_summary['recheck_5fold_ndcg_at_10']:.6f}",
    )

    print(
        "5-Fold MAP@10:",
        f"{best_summary['recheck_5fold_map_at_10']:.6f}",
    )

    print(
        "5-Fold HR@10:",
        f"{best_summary['recheck_5fold_hit_rate_at_10']:.6f}",
    )

    print(
        "Candidate:",
        (
            f"Size{int(best_summary['candidate_size'])} | "
            f"I{int(best_summary['item_n'])} "
            f"B{int(best_summary['bpr_n'])} "
            f"C{int(best_summary['content_n'])} "
            f"U{int(best_summary['user_n'])}"
        ),
    )
    ...
    print(
        "Candidate Recall:",
        f"{best_summary['candidate_pool_recall']:.6f}",
    )
    ...
    print(
        "3) Guardrail을 만족한 모델 중 actual Macro NDCG@10을 최대화했습니다."
    )

    print(
        "4) HR@10 / MAP@10 / Micro P/R / Candidate Recall도 함께 기록했습니다."
    )

    print(
        "5) Candidate Size Search = 90~130, step=1."
    )
    ...
    print(
        "7) Final400은 아직 사용하지 마세요."
    )

    print(
        "8) 이 실험의 Top10 5-fold 결과에서 NDCG형 / Hit-Recall형 / 안정형 / 균형형 / Retrieval형 후보를 4~5개 선정하세요."
    )

    print(
        "9) 후보 선정 후 Feature Ablation과 안정성 검증을 진행하세요."
    )
