# hybrid_arctech_experiment/exp_u_five_candidate_feature_ablation.py
# ============================================================
# Experiment U - Five Candidate Feature Ablation
#
# 목적
# ------------------------------------------------------------
# Final Joint Optuna 90~130의 5-Fold 결과에서 선정한
# 서로 성격이 다른 최종 후보 5개를 대상으로 Feature Ablation을 수행한다.
#
# 후보:
# A. NDCG형      = Trial 2
# B. Recall형    = Trial 57
# C. Precision형 = Trial 3
# D. 안정형      = Trial 67
# E. HR형        = Trial 60
#
# 각 후보에서 고정하는 것:
# - Candidate Size
# - Retriever Ratio / Quota
# - XGBoost hyperparameters
# - LTR users = 1000
# - 동일한 5-Fold user split
#
# 바꾸는 것:
# - Feature만 하나씩 제거
#
# Optuna feature setting 기준:
# - OFF였던 feature를 새로 추가하지 않는다.
# - 실제로 ON인 feature/group만 하나씩 제거한다.
#
# A / E (16 features):
#   Full
#   - Item Flag
#   - Popularity Affinity
#   - Retriever Count
#   Full14 Reference (Item Flag + Popularity Affinity 동시 제거)
#
# B / C / D (14 features):
#   Full
#   - Retriever Count
#
# 총:
# - strict one-at-a-time ablation = 14 configs
# - Full14 reference 2개 포함 = 16 configs
# - 5-Fold = 80 XGBoost fits
#
# 평가:
# - Total Hits
# - Macro P@10
...
# - Candidate / XGB params는 후보별 Optuna 결과 그대로 고정한다.
# - 각 fold에서는 동일 training procedure(early stopping)를 사용한다.
# ============================================================
...
def main():
    ...
    print(
        " Final Joint Optuna Top Models -> Fixed Candidate / Fixed XGB / Feature-Only Ablation"
    )

    print(
        " LTR users = 1000 | 5-Fold | Optuna = OFF | Final400 = NOT USED"
    )
    ...
    # ========================================================
    # 1. Validate candidate definitions
    # ========================================================

    validate_five_candidate_configs()


    # ========================================================
    # 2. Count planned experiments
    # ========================================================
    ...
    if len(
        ...
        print(
            "WARNING: 현재 LTR users가 1000명이 아닙니다:",
            len(
                ltr_user_ids
            ),
        )
    ...
    # ========================================================
    # 5. Train / Test + positive test set
    # ========================================================
    ...
    for model_key, candidate in (
        ...
        for variant_name, feature_columns in (
            ...
            print(
                (
                    f"[{experiment_index}/{total_configs}] "
                    f"{model_key} | {variant_name} "
                    f"| features={len(feature_columns)}"
                )
            )
    ...
    delta_df = add_delta_vs_full(
        summary_df
    )


    # Pure ablations 먼저,
    # full14_reference는 reference라 뒤쪽.
    variant_order = {
        "full": 0,
        "minus_item_flag": 1,
        "minus_item_agreement": 2,
        "minus_rrf": 3,
        "minus_rank_std": 4,
        "minus_pop_affinity": 5,
        "minus_retriever_count": 6,
        "full14_reference": 7,
    }
    ...
    # ========================================================
    # 10. Feature importance interpretation table
    #
    # Ablation에서 제거 후 metric이 감소하면:
    # full - ablated > 0 = feature가 기여한 것.
    # 아래 impact는 보기 편하게 부호를 뒤집어 저장.
    # ========================================================
    ...
    planned_path = (
        ABLATION_RESULT_DIR
        /
        "five_candidate_ablation_plan.csv"
    )

    summary_path = (
        ABLATION_RESULT_DIR
        /
        "five_candidate_ablation_summary.csv"
    )

    fold_path = (
        ABLATION_RESULT_DIR
        /
        "five_candidate_ablation_folds.csv"
    )

    delta_path = (
        ABLATION_RESULT_DIR
        /
        "five_candidate_ablation_delta_vs_full.csv"
    )

    impact_path = (
        ABLATION_RESULT_DIR
        /
        "five_candidate_feature_impact.csv"
    )

    config_path = (
        ABLATION_RESULT_DIR
        /
        "five_candidate_configs.json"
    )
        ...
        json.dump(
            {
                ...
                "method":
                    (
                        "Candidate / Ratio / XGB params fixed per model. "
                        "Only features are removed. "
                        "Same 5-fold user split for every candidate/variant."
                    ),
            },
            file,
            ensure_ascii=False,
    ...
    print(
        " FIVE CANDIDATE FEATURE ABLATION SUMMARY"
    )
    ...
    print(
        "1) Optuna는 사용하지 않았습니다."
    )

    print(
        "2) 각 후보의 Candidate Size / Ratio / XGB hyperparameters는 고정했습니다."
    )

    print(
        "3) 실제 ON인 feature/group만 하나씩 제거했습니다."
    )

    print(
        "4) Full14 reference는 one-at-a-time ablation이 아니라 참고용 조합 실험입니다."
    )

    print(
        "5) 모든 모델/variant가 동일한 LTR1000 5-Fold split을 사용합니다."
    )
