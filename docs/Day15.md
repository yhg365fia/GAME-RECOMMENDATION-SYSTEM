...
오늘은 Item-Based CF의 정량적 우위를 6개 실험으로 직접 확인하면서, Content-Based와 Item-Based가 **동일한 입력에서도 정반대 방향으로 쏠린다는 사실**을 발견함으로써 두 모델이 "누가 더 낫다"가 아니라 **서로 다른 종류의 신호를 포착하고 있다는 것**을 데이터로 확인한 날이었다.